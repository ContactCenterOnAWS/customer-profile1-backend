...
```bash
customer-profile1-backend$ sam build --use-container
```
...
```bash
customer-profile1-backend$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
customer-profile1-backend$ sam local start-api
customer-profile1-backend$ curl http://localhost:3000/
```
...
```bash
customer-profile1-backend$ sam logs -n HelloWorldFunction --stack-name customer-profile1-backend --tail
```
...
```bash
customer-profile1-backend$ pip install -r tests/requirements.txt --user
# unit test
customer-profile1-backend$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
customer-profile1-backend$ AWS_SAM_STACK_NAME="customer-profile1-backend" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "customer-profile1-backend"
```
